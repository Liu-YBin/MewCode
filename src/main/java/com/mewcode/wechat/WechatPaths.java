...
public final class WechatPaths {
    ...
    public static Path root() {
        String configured = System.getProperty("mewcode.wechat.dir");
        if (configured == null || configured.isBlank()) {
            configured = System.getenv("MEWCODE_WECHAT_DIR");
        }
        if (configured == null || configured.isBlank()) {
            configured = Path.of(System.getProperty("user.home"), ".mewcode", "wechat").toString();
        }
    ...
    public static Path pidFile() {
        return root().resolve("mewcode-wechat.pid");
    }
}
