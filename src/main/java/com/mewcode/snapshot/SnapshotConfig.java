...
public record SnapshotConfig(
    ...
    public static SnapshotConfig fromEnvironment() {
        boolean enabled = readBoolean("mewcode.snapshot.enabled", "MEWCODE_SNAPSHOT_ENABLED", true);
        Path root = Path.of(readString("mewcode.snapshot.dir", "MEWCODE_SNAPSHOT_DIR",
                Path.of(System.getProperty("user.home"), ".mewcode", "snapshots").toString()));
        int max = readInt("mewcode.snapshot.max", "MEWCODE_SNAPSHOT_MAX", 50);
        List<String> excludes = mergeExcludes(readString("mewcode.snapshot.excludes", "MEWCODE_SNAPSHOT_EXCLUDES", ""));
