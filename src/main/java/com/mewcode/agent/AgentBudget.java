 ...
 * 1. 系统属性：{@code mewcode.react.token.budget} / {@code mewcode.react.stagnation.window} /
 *    {@code mewcode.react.hard.max.iterations}
 ...
 * 场景（CI / 自动化批跑）通过 {@code -Dmewcode.react.token.budget=N} 显式启用。
...
public class AgentBudget {
    ...
    public static AgentBudget fromLlmClient(LlmClient llmClient) {
        ...
        // 显式 -Dmewcode.react.token.budget=N 仍可启用硬预算，覆盖默认。
        return new AgentBudget(
                readIntProperty("mewcode.react.token.budget", Integer.MAX_VALUE),
                readIntProperty("mewcode.react.stagnation.window", DEFAULT_STAGNATION_WINDOW),
                readIntProperty("mewcode.react.hard.max.iterations", DEFAULT_HARD_MAX_ITERATIONS)
        );
    }
