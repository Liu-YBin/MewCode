...
import com.mewcode.context.ContextProfile;
import com.mewcode.context.TokenUsageFormatter;
import com.mewcode.image.ImageReferenceParser;
import com.mewcode.llm.LlmClient;
import com.mewcode.llm.LlmTraceLogger;
import com.mewcode.lsp.LspDiagnosticReport;
import com.mewcode.memory.ConversationHistoryCompactor;
import com.mewcode.memory.ExplicitMemoryHints;
import com.mewcode.memory.MemoryManager;
import com.mewcode.prompt.ProjectMemoryLoader;
import com.mewcode.prompt.PromptAssembler;
import com.mewcode.prompt.PromptContext;
import com.mewcode.prompt.PromptMode;
import com.mewcode.render.PlainRenderer;
import com.mewcode.render.Renderer;
import com.mewcode.render.StatusInfo;
import com.mewcode.runtime.CancellationContext;
import com.mewcode.skill.SkillContextBuffer;
import com.mewcode.skill.SkillIndexFormatter;
import com.mewcode.skill.SkillRegistry;
import com.mewcode.tool.ToolRegistry;
import com.mewcode.tool.ToolRegistry.ToolExecutionResult;
import com.mewcode.tool.ToolRegistry.ToolInvocation;
import com.mewcode.util.AnsiStyle;
import com.mewcode.util.TerminalMarkdownRenderer;
...
public class Agent {
    ...
    public String getContextStatus() {
        com.mewcode.context.ContextProfile profile = memoryManager.getContextProfile();
        ...
        for (LlmClient.Message msg : conversationHistory) {
            int t = com.mewcode.memory.TokenBudget.estimateMessagesTokens(List.of(msg));
            switch (msg.role()) {
    ...
    private int estimateToolsSchemaTokens() {
        try {
        return com.mewcode.memory.MemoryEntry.estimateTokens(
    ...
    private long estimateCurrentContextTokens() {
        long messageTokens = com.mewcode.memory.TokenBudget.estimateMessagesTokens(conversationHistory);
    ...
    private void logRequestContext(String scope, List<LlmClient.Tool> tools) {
        ...
        for (int messageIndex = 0; messageIndex < conversationHistory.size(); messageIndex++) {
            ...
            messages++;
            int tokens = com.mewcode.memory.TokenBudget.estimateMessagesTokens(List.of(msg));
        ...
        if (tools != null && !tools.isEmpty()) {
            try {
            toolsSchemaTokens = com.mewcode.memory.MemoryEntry.estimateTokens(
